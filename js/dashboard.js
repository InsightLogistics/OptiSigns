...
document.addEventListener('DOMContentLoaded', () => {
    const setupChart = (chartId, type, datasets, additionalOptions = {}) => {
        ...
        if (ctx) {
            ...
            const options = deepMerge(defaultOptions, additionalOptions);
    ...
    const weatherIconUrl = (status) => {
        ...
        return `https://placehold.co/80x80/cccccc/ffffff?text=Icon`;
    };
        ...
        try {
            ...
            if (forecastTableContainer) {
                ...
                if (forecastWeatherData.length > 0) {
                    ...
                    const nextFiveDaysForecast = forecastWeatherData.slice(1, 6);
                    const numForecastDays = 5;

                    for (let i = 0; i < numForecastDays; i++) {
                        const day = nextFiveDaysForecast[i];
                        const th = document.createElement('th');
                ...
                exchangeRateDatasets, {
                    scales: {
                        x: {
                            ...
                            ticks: { autoSkipPadding: 10, maxTicksLimit: undefined },
                            title: { display: false }
                        },
                        y: {
                            beginAtZero: false,
